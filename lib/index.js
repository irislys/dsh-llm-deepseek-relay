...
// registers one official `DeepSeekAdapter` route per configured provider, each
// pointing at that provider's baseURL. The same official model may appear under
// several providers (each with its own baseURL / key / relayId): the llm
// service keys every selection by (provider, model), so duplicate model ids
// across provider groups are safe and resolve to their owning provider. The
...
/**
 * A DeepSeekAdapter whose provider route shows the provider's own name in the
...
export function apply(ctx, pluginConfig) {
	...
	// Register one adapter per provider. A provider's baseURL / models / key are
	...
	for (const p of currentDoc.providers) {
		const adapterOptions = () => {
			...
			assertLiveProvider(live, p.provider, configPath)
		...
		const adapter = new RelayAdapter(
			{
				...
				// dsh 0.1.2-alpha added an unconditional prepareExtensions call on
				// the stream path; the official adapter wires it to the
				// deepseekLlmApiExtensions service (plugin/version reporting,
				// session-log upload). Reuse that service when present, otherwise
				// fall back to a no-op so relayed requests keep working.
				prepareExtensions: (request) =>
					ctx.get('deepseekLlmApiExtensions')?.prepare(request) ?? Promise.resolve({ fields: {}, accept: () => Promise.resolve() }),
			},
			p.displayName,
		)
	...
	// Cordis collects a returned function as the bundle disposer: stop the
	// config watcher on unload / patch reload, so hot-reloaded bundles never
	// pile up stale watchers (each holding a disposed ctx and re-reading the file).
	return () => {
		unwatchFile(configPath)
	}
}

/** A provider route vanished from the live config (topology change needs restart). */
function assertLiveProvider(live, provider, configPath) {
	if (live === undefined) {
		throw new LlmError(
			`llm-deepseek-relay: provider route "${provider}" was removed or renamed in ${configPath}; restart dsh to apply provider topology changes`,
			'UNKNOWN_PROVIDER',
		)
	}
}
