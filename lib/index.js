...
export const Config = z.object({
	...
	routeMode: z.union(['always', 'model-match', 'off']).default('model-match'),
})
...
function normalizeConfig(doc) {
	...
	const routeMode = cfg.routeMode ?? 'model-match'
...
export function apply(ctx, pluginConfig) {
	...
	// Optional steering: on each conversation request, if the selected model
	// matches a configured official id (or a relay id), route to its supplier.
	if (currentDoc.routeMode !== 'off') {
		ctx.on('agent/request', async (_payload, next) => {
			...
			for (const p of currentDoc.providers) {
				for (const m of p.models) {
					byOfficial.set(m.official, { provider: p.provider, relayId: m.relayId })
					byRelay.set(m.relayId, p.provider)
				}
			}
			let hit
			if (byOfficial.has(cfg.model)) hit = byOfficial.get(cfg.model)
			else if (byRelay.has(cfg.model)) hit = { provider: byRelay.get(cfg.model), relayId: cfg.model }
			if (hit !== undefined && (hit.provider !== cfg.provider || hit.relayId !== cfg.model)) {
				return { ...cfg, provider: hit.provider, model: hit.relayId }
			}
			return cfg
		})
	}
